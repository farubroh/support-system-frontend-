...
export class DeveloperDashboardComponent implements OnInit {
  ...
  drop(e: CdkDragDrop<Card[]>) {
    ...
    // Prevent moving a card out of "COMPLETED" status
    if (fromKey === 'COMPLETED' && toKey !== 'COMPLETED') {
      return; // Do nothing if trying to move from COMPLETED to another status
    }
  ...
  openCard(card: Card) {
    ...
    console.log("adfw: ",this.selected());
